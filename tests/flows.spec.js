...
async function bootApp(page) {
  ...
  await expect
    .poll(() => page.evaluate(() => window.__IREM_RUNTIME__))
    .toBe("esm");

  const webglSupported = await page.evaluate(() =>
    Boolean(window.SUPPORT_WEBGL),
  );
  ...
  await expect
    .poll(
      () =>
        page.evaluate(() =>
          document.querySelector(".app")?.classList.contains("show"),
        ),
      {
        timeout: 15_000,
      },
...
test("nav add button opens the upload-methods step", async ({ page }) => {
  ...
  await expect(page.locator(".add-steps-upload-methods")).toHaveCSS(
    "display",
    "block",
  );
  await expect(
    page.locator(".add-steps-upload-methods-selection-container"),
  ).toBeVisible();

  // Wait for the step transition animation to settle.
  await page.waitForTimeout(1000);

  await expect(page).toHaveScreenshot("after-add-click.png", {
...
test("nav search button opens the search overlay", async ({ page }) => {
  ...
  await expect(page).toHaveScreenshot("after-search-click.png", {
    maxDiffPixelRatio: 0.02,
  });
...
test("footer excludes retired campaign links", async ({ page }) => {
  ...
  await expect(page.locator(".footer-link-credits")).toHaveCount(0);
